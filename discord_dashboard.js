...
// Forgot password (look up account by email) ────────────────────────────────
app.post("/auth/forgot-password", (req, res) => {
  const email = (req.body.email || "").toLowerCase().trim();
  const user  = email ? usersDB[email] : null;
  if (!user) return res.json({ found: false });
  return res.json({ found: true, username: user.username });
});

// Reset password (email + new password, no token/email step needed) ────────
app.post("/auth/reset-password", async (req, res) => {
  ...
  await sendMail(email, "Your Dashboard password was changed",
